...
static void queue_network_event(network_event_type_t type, int32_t event_id)
{
    ...
    if (xQueueSend(s_network_event_queue, &event, 0) != pdTRUE) {
        ESP_LOGE(TAG, "network event queue is full; dropped event %d", type);
    }
}
...
static esp_err_t root_get_handler(httpd_req_t *req)
{
    ...
    if (err != ESP_OK) {
        ap_count = 0;
        ESP_LOGD(TAG, "Wi-Fi scan failed: %s", esp_err_to_name(err));
    }
...
static esp_err_t connect_post_handler(httpd_req_t *req)
{
    ...
    ESP_LOGD(TAG, "saving Wi-Fi credentials for SSID '%s'", ssid);
                               ...
                               int32_t event_id, void *event_data)
{
    ...
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_DISCONNECTED) {
        ...
        if (s_retry_count < CONNECT_MAX_RETRIES) {
            s_retry_count++;
            ESP_LOGD(TAG, "Wi-Fi disconnected; retry %d/%d", s_retry_count,
                     CONNECT_MAX_RETRIES);
            queue_network_event(NETWORK_EVENT_LOST, event_id);
            esp_wifi_connect();
        } else {
            ESP_LOGD(TAG, "saved Wi-Fi is unavailable; starting setup AP");
            queue_network_event(NETWORK_EVENT_LOST, event_id);
...
{
    ESP_LOGD(TAG, "manual Wi-Fi reset requested");
